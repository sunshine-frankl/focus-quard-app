...
def get_notifier():
    class _N:
        ...
        def ok(self):
            return bool(TELEGRAM_BOT_TOKEN) and _req is not None
...
def process_frame(img, state, settings):
    """Process a single BGR frame, return annotated frame + updated state."""
    h, w = img.shape[:2]
    ...
    if results.multi_face_landmarks:
        for face_lm in results.multi_face_landmarks:
            ...
            for iris_idx in [L_IRIS_IDX, R_IRIS_IDX]:
                cv2.circle(ann, (int(lm[iris_idx].x*w), int(lm[iris_idx].y*h)), 5, (255,80,80), -1)
            ...
            state["gaze_buf"].append(avg_ratio)
            smooth = sum(state["gaze_buf"]) / len(state["gaze_buf"])
    ...
    for t in list(state["vio_first"]):
        if t not in active_types:
            del state["vio_first"][t]

    for vtype, vtext in active:
        if vtype not in state["vio_first"]:
            state["vio_first"][vtype] = now
            continue
        if now - state["vio_first"][vtype] < grace.get(vtype, 0.6):
            continue
        if now - state["vio_sent"].get(vtype, 0) < VIOLATION_COOLDOWN:
            continue
        state["vio_sent"][vtype] = now
        ts = time.strftime("%H:%M:%S")
        state["violations_log"].appendleft(f"[{ts}] {vtext}")
    ...
    if person_absent:
        ...
        status, color = "🟢 Focused",    "#00ff9d"
    elif score > 55:
        status, color = "🟡 Drifting",   "#ffcc00"
    else:
        status, color = "🔴 Not focused","#ff4444"
    ...
    put(f"Focus {int(score)}%", 28, score_col)
    put(f"Gaze  {gaze_cv}",     52, (220,220,220))
    put(f"Faces {faces_count}", 76, (220,220,220))
...
def login_page():
    ...
    with col:
        ...
        if c1.button("🛡️ Admin",    use_container_width=True):
            db = get_db(); u = db["users"]["admin"]
            st.session_state.update({"authenticated":True,"username":"admin","display_name":u["name"],"role":u["role"]}); st.rerun()
        if c2.button("👨‍🏫 Teacher", use_container_width=True):
            db = get_db(); u = db["users"]["teacher"]
            st.session_state.update({"authenticated":True,"username":"teacher","display_name":u["name"],"role":u["role"]}); st.rerun()
        if c3.button("🎓 Student",  use_container_width=True):
            db = get_db(); u = db["users"]["student"]
            st.session_state.update({"authenticated":True,"username":"student","display_name":u["name"],"role":u["role"]}); st.rerun()
        ...
        if st.button("Sign in", use_container_width=True, type="primary"):
            db = get_db(); u = db["users"].get(username)
            if u and u["password"] == _hash(password):
                st.session_state.update({"authenticated":True,"username":username,"display_name":u["name"],"role":u["role"]}); st.rerun()
            else:
        ...
        if st.button("Add user", type="primary"):
            if new_user and new_pass and new_name:
                if new_user in db["users"]:
                    ...
                    db["users"][new_user] = {"name":new_name,"password":_hash(new_pass),"role":new_role}
                    st.success(f"User **{new_user}** created"); st.rerun()
            else:
        ...
        if not db["exams"]:
            ...
            for eid, ex in db["exams"].items():
                st.markdown(f"""<div class="exam-card">
                    ...
                    &nbsp;·&nbsp; {ex['created_at']}</small></div>""", unsafe_allow_html=True)
...
def teacher_page():
    ...
    with tab_create:
        ...
        students = {u: info["name"] for u, info in db["users"].items() if info["role"] == "student"}
        if not students:
            st.warning("No students registered yet.")
        else:
            title      = st.text_input("Exam title", placeholder="e.g. Midterm Exam — Math")
            student_un = st.selectbox("Assign to student", options=list(students.keys()),
            ...
            if st.button("📋 Create exam", type="primary", use_container_width=True):
                if not title:
                    ...
                    db["exams"][eid] = {
                        "title": title, "teacher": uname, "student": student_un,
                        "created_at": time.strftime("%Y-%m-%d %H:%M"),
                        "status": "pending", "telegram": enable_tg, "result": None,
                    }
                    st.success(f"✅ Exam **{title}** created"); st.rerun()

    with tab_results:
        my_exams = {eid: ex for eid, ex in db["exams"].items() if ex["teacher"] == uname}
        if not my_exams:
            ...
            for eid, ex in my_exams.items():
                st.markdown(f"""<div class="exam-card">
                    <b>{ex['title']}</b> &nbsp;
                    <span class="badge-{ex['status']}">{ex['status'].upper()}</span><br>
                    <small>Student: <b>{ex['student']}</b> &nbsp;·&nbsp; {ex['created_at']}</small>
...
# ══════════════════════════════════════════════════════════════════════════════
#  STUDENT PAGE  (uses st.camera_input — no WebRTC needed)
# ══════════════════════════════════════════════════════════════════════════════
def student_page():
    db = get_db()
    ...
    # Init session state for this exam
    state_key = f"exam_state_{eid}"
    if state_key not in st.session_state:
        st.session_state[state_key] = {
            ...
            "violations_log": deque(maxlen=20),
            "vio_first":      {},
            "vio_sent":       {},
            "gaze_buf":       deque(maxlen=6),
            "focus_score":    0,
            "gaze_ui":        "—",
            "blink_rate":     0.0,
            "session_time":   0,
    ...
    with col_side:
        ...
        if st.button("✅ Submit exam", type="primary", use_container_width=True):
            fs = list(state["focus_scores"])
            vlog = list(state["violations_log"])
            db["exams"][eid]["result"] = {
    ...
    # Process frame if camera captured one
    if camera_image is not None:
        file_bytes = np.frombuffer(camera_image.getvalue(), np.uint8)
        img = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)
        if img is not None:
            ann, state = process_frame(img, state, settings)
            st.session_state[state_key] = state
            # Show annotated frame
            ann_rgb = cv2.cvtColor(ann, cv2.COLOR_BGR2RGB)
            col_cam.image(ann_rgb, use_container_width=True)

    # Show metrics
    ph_focus.metric("🎯 Focus",      f"{int(state['focus_score'])}%")
    ph_time.metric( "⏱ Session",    f"{int(state['session_time'])} s")
    ph_blink.metric("👁 Blinks/min", f"{state['blink_rate']:.1f}")
    ph_gaze.metric( "👀 Gaze",       state["gaze_ui"])
    ph_status.markdown(
        f"<h3 style='color:{state['color']};margin:0'>{state['status']}</h3>",
        unsafe_allow_html=True)
    ...
    if vlog:
        ph_viol.markdown("".join(f'<div class="vrow">{v}</div>' for v in vlog[:8]),
                         unsafe_allow_html=True)
    elif state["active_violations"]:
        ph_viol.markdown("".join(f'<div class="vrow">{v}</div>' for v in state["active_violations"][:8]),
                         unsafe_allow_html=True)
    else:
    ...
    if len(fs) > 2:
        ...
        fig.update_layout(
            ...
            yaxis=dict(range=[0,100], ticksuffix="%",
                       gridcolor="rgba(255,255,255,0.05)", tickfont=dict(color="#aaa")),
            xaxis=dict(showgrid=False, showticklabels=False))
        chart_ph.plotly_chart(fig, use_container_width=True, key=f"sc_{eid}")
